...
#i 先行研究/* TODO: ref */では, MPEG 動画から画像を複合せずにマクロブロック (映像の処理単位) から動画のカットを試みている.
しかしこの方法では, MPEG 動画のみにしか対応ができない. 私たちは, MPEG 動画だけでなく, カメラ映像からリアルタイムに 解析, 抽出
...
#articleFig(
  ...
  imgWidth: 82mm,
  imgCaption: [システム全体図 <overall_view>],
)[
...
#grid(
  ...
  gutter: 6pt,
  [
    #i 既存の論文を参考にし, 動物行動を 8 つに分類した (表 1). しかし, 長時間の動画をそのまま AI で分類することは, データ量が大きく難しい.
    そのためまずは, 必要な動きのある場面のみを抽出することにした. 
  ...
  [#figure(align(center)[#table(
        ...
        [$C$],
        [交尾],
      )], caption: [動物行動の分類], supplement: "表")
  ],
)


#i 開発当初は, 静的なしきい値で切り抜きを行っていたが, 動画によって最適なしきい値が異なるため, 過去の傾きから指数移動平均を算出し,
動的なしきい値を使用した切り抜きを行う方法に変更した. プログラムの処理内容を図 3 に示す.
...
#articleFig(
  imgPath: "../assets/img/viewer_visual.png",
  imgWidth: 82mm,
  imgCaption: [Viewer の画面 <viewer_visual>],
)[
   
  #i オブジェクトストレージからファイルを読み込み, AI が生成した説明やタグをわかりやすく表示した (図 n). Material Design
  を一部採用し, ニューモフィズムと丸みを持たせたデザインにすることによって操作しやすく親しみやすいアプリにした.
...
#show raw: it => text(font: "UDEV Gothic 35JPDOC", it)
...
#articleFig(
  imgPath: "../assets/img/zoo.png",
  imgWidth: 80mm,
  imgCaption: [東山動物園での意見交換 <zoo>],
)[ 
  #i 2023 年 10 月 22 日 (日) 東山動物園にご協力いただき, 飼育を担当されている 3 名の方にシステムのプレゼンテーションを行った.
  飼育員の方のご意見では, 今回使用した分類の方法ではあいまいな点が多く, 動物行動の定義付けに関する難しさをご指摘いただいた. そのため,
...
= 今後の課題
#i 情報技術の進歩は日進月歩であり, 長時間の映像から必要な部分を効率的に抽出するシステムの需要は今後, 高まるはずである. この需要に応えるため,

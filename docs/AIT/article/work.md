...
また、 イベントループ抽象クラスのコンストラクタにシングルトンストアを注入することで、 継承しているレンダラーやモデル内で簡単にシングルトンストアを使用可能になり、 モデル内で安全にステートの読み書きができるようにした。これらの設計の工夫によって、後続のモデルにデータを渡しやすくなった。
リアルタイム性に優れた OpenCV や PyQtGraph、 PySide などの適切なフレームワークを組み合わせて表示できた。
...
動物によって必要なソースや、必要なカメラの数などが変わる。そのため、
...
過去に撮りためた動画から特定の行動を抽出する場合はこのままでも使うことができるが、リアルタイムだとまだ足りない部分がある。
...
繁殖と関連づける際は発情兆候が 1 日のうちに起こったかという回数も重要になる。
普段とは異なる行動も抽出することが可能なら、動物の発情の度合いも分かる。

...
レンダラーとモデルが事前に定義したインターフェースを継承して、切り抜きを行うプログラムを作成した。レンダラー抽象クラスは、
...
類似度ハッシュと合わせて観察対象のみの動きを捉えることができた。しかし、実際に作成し、動作を確認したところ YOLO v8 では処理が遅く動画の書き出しに影響が出たため、類似度ハッシュのみをトリガーとしたが、

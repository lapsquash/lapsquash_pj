...
本研究では Raspberry Pi で対象の動物が動いている場面のみを抽出し、ストレージに保存。抽出した場面に AI で説明と対応するタグをつける。将来の目的としては動物に対する知識を得るアプリとしての確立、動物行動以外の分野での応用を目的としている。
...
現在の目的が「動物について知識を得ることができるアプリ制作」である場合、「飼育員さんの業務の効率化」とずれている。
飼育員さんは複数のカメラの映像を分析・分類可能にすることや、映像のズーム、ズームアウト機能、小さな動物の切り抜き精度の向上、音声の分析・分類機能(行動によって声の出し方が異なっており、「喧嘩」と「遊び」、「睡眠」と「座る」の区別をするため。)を必要としている。

動物によって必要なソースや、必要なカメラの数など変わる。そのため、
1 つのアプリケーションで複数の動物に対応させるのは難しい。用途によって見たい行動や、目的も変わってくるため、目的ごとにアプリを分けて使用者が選択できる方式にすればいい、という意見をいただいた。

動物行動の研究などでは必ずしも「動いている場面」ばかリを切り抜くわけではなく、見る目的によっては「動いていない場面」を切り抜く場合がある。

1 つの動物で複数の定点カメラがある場合を想定していなかった。

長い中で、特定の場面を抽出するような作業はそもそも必要ない。

過去に撮りためたビデオデータをその中から行動を切り抜く場合はこのままでも使うことができるが、リアルタイムだとまだ足りない部分がある。

担当者にしか見せない姿もあるが、逆に担当者には見せない姿も知ることができる。思わぬ発見ができる。

不審な動きをした人の検出など監視システムとしての利用方法

動物の体温や気温の目を向けると、サーモカメラに対応させなければならない。

その動物にしか聞こえないような音域や、超音波、紫外線、赤外線など人間には検知できない物にも対応させることができたら、動物だけでなく、昆虫など他の分野にも応用させられる可能性がある。

魚類などの常に動いている動物が対象の場合、「動いているか」ではなく、行動の部分のみをピックアップできれば水中の動物や止まらない動物にも利用できる。
カメラの台数を増やすことが必要。

1 方向の動画のみで判断するのではなく、上からの映像と横からの映像と 2 方向の映像を使うとより精度が上がる。1 方向のみの動画では起きているのか、寝ているのか区別がつかない場合が多い。
...
行動観察の場合、 1 日の流れの中の何％をその行動に費やしてるかの割合を調べることが多く、
...
動物の出産後、担当者でもその様子を覗きにいくのはシビアで、親が「ここは子育ての環境にない」と判断し、育児を放棄してしまう可能性がある。育児を放棄された子どもは人工保育になる。人口で育てた子どもよりも親が育てた子どもの方が丈夫であるため、育児放棄をされないよう、カメラで沢山の情報を入手する必要がある。子どもの数、授乳の回数や音などを拾うことができれば、子どもの死亡やけがのリスクを下げることができる。
...
動物園の職員の方々の話を聞いた際、「画面の動きだけでなく、音の分析・分類ができると動物の観察に役立つ」とアドバイスをいただいた。音声の分析についてはあまり重要性を感じなかったが、動物の鳴き声は種類、性別、年齢、時期などで鳴き方や声色が変わるが分かった。今後の研究では音声の分析、夜間など暗い中での分析、分類に力を入れていく。
現状、切り抜き範囲が細かい、 AI に動画の説明をさせてみたところ、。「水槽の中にいる猿」のような文章しか生成できなかった。  
